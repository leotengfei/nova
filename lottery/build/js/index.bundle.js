...
/***/ function(module, exports, __webpack_require__) {
	...
	Zepto(function() {
        ...
        //****************************************自定义编辑区域（提交按钮）**************************************************
		localStorage['isSubed']=0;
		$('#stel').on('blur', function () {
			var regname=/^[\u3e00-\u9fa5]{2,4}$/ig;
			var bsname=regname.test($('#sname').val());
			if(!bsname){
				$('#tips').html("姓名格式有误！");
			}
			var regtel=/^[1][3,4,5,7,8][0-9]{9}$/ig;
			var btel=regtel.test($('#stel').val());
			if(!btel){
				$('#tips').html("手机号格式有误！");
			}
			if(!bsname&&!btel){
				$('#tips').html("请检查姓名和手机号格式！");
			}
			console.log(bsname,btel);
			//确保一个客户端只提交一次
			if(localStorage['isSubed']==0&&bsname&&btel){
				submsg();
			}
		});
...
//自己添加的js
	...
	function generatePoster() {
		...
		var y1 = $frame.width()/4;
		var r1 = 30;
		var x2 = $frame.width()/2;
		var y2 = $frame.width()/4+100;
		var r2 = 150;
		var radialGradient1 = ctx.createRadialGradient(x1, y1, r1, x2, y2, r2);
		radialGradient1.addColorStop(0, 'rgba(255,255,255,0.3)');
		radialGradient1.addColorStop(1, $('input[name=cColor]:checked').val());
		ctx.fillStyle = radialGradient1;//绘制背景色
		ctx.fillRect(0,0,canvas.width,canvas.height);
		...
		var txt=$('#tname').val()+":";
		var txtMsg=$('#msg').val();
		var fSize=($frame.width())/25;
		console.log(fSize);
		ctx.font=fSize+'px Hiragino Sans GB,KaiTi,SimSun';
		ctx.textBaseline='top';
		//文字颜色
		//console.log($('input[name=fColor]:checked').val());
		ctx.fillStyle=$('input[name=fColor]:checked').val();
		ctx.fillText(txt,fSize*2,fSize*10);
		var lineWidth = 0;
		...
		for(var i=0;i<txtMsg.length;i++){
			...
			if(lineWidth>($frame.width()-fSize*12)){//减去initX,防止边界出现的问题
				ctx.fillText(txtMsg.substring(lastSubStrIndex,i),fSize*2,initY);
				initY+=lHeight;
				lineWidth=0;
				lastSubStrIndex=i;
			}
			if(i==txtMsg.length-1){
				ctx.fillText(txtMsg.substring(lastSubStrIndex,i+1),fSize*2,initY);
			}
		}
