...
$('#registerName').blur(function () {
    ...
    valiRegisterName=regname.test($(this).val());
    if(!valiRegisterName){
        $('#loginMsg').html("<span class='glyphicon glyphicon-minus-sign'></span>请确保姓名为2-5个汉字!");
    }else{
        $('#loginMsg').html('&nbsp;');
    }
});
            ...
            success: function (data) {
                if(data.code==400){
                    ...
                    if(sessionStorage['uid']===undefined){
                        ...
                        location.href='choosePaper.html';
                    }
...
$('#btnRegister').click(function () {
    ...
    if(valiRegisterTel&&valiRegisterUpwd&&valiRegisterUpwdAagin&&valiRegisterName){
        console.log(registerTel,registerUpwd);
        $.ajax({
            type:'POST',
            url:'data/register.php',
            data:{registerTel:registerTel,registerUpwd:registerUpwd,registerName:registerName},
            success: function (data) {
                if(data.code==400){
                    $('#loginMsg').html('注册失败！');
                }else if(data.code==401){
                    $('#loginMsg').html('该电话号码已被占用！');
                }else if(data.code==200){
                    location.href='choosePaper.html';
                    //alert(data.uid);
